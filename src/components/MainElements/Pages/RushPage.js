...
class RushPage extends React.Component {
  ...
  componentDidMount() {
    this.setState({
      table: localStorage.getItem("rushTable")
        ? JSON.parse(localStorage.getItem("rushTable"))
        : this.state.table,
    });
  }
    ...
    if (this.state.new) {
      if (modal.supplier !== "Склад") {
        ...
        const suppliers = JSON.parse(
          localStorage.getItem("supplierCatalogTable")
        );
  ...
  render() {
    ...
    return (
      <main className="main main-rush">
        ...
        <RushTable data={tableData} handleRowClick={this.handleRowClick} />
        <RushModal
