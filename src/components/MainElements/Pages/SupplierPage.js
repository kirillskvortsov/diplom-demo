...
class SupplierPage extends React.Component {
  constructor(props) {
    ...
    this.handleFormDeleteButtonClick = this.handleFormDeleteButtonClick.bind(
      this
    );
  ...
  componentDidMount() {
    this.setState({
      table: localStorage.getItem("supplierTable")
        ? JSON.parse(localStorage.getItem("supplierTable"))
        : this.state.table,
  ...
  handleInput() {
    ...
    copy = copy.filter((item) => {
      let date =
        item.date1.substring(8, 10) +
        "." +
        item.date1.substring(5, 7) +
        "." +
        item.date1.substring(0, 4);
      return (
  ...
  handleRowClick(id) {
    let copy = cloneDeep(this.state.table);
    for (let i = 0; i < copy.length; i++) copy[i].selected = false;
  ...
  handleSaveButtonClick(e, bool) {
    ...
    if (this.state.new) {
      ...
      if (
        modal.status === "Отправлено" &&
        this.state.table[tableId].status === "Ожидает отправки"
      ) {
    ...
    return (
      <main className="main main-rush">
        ...
        <div className="rush-page-searchbar">
          <Button
            onClick={() => this.handleNewButtonClick(true)}
            className="rush-btn rush-btn-new"
          >
            Новый заказ
          </Button>
          <Button
            onClick={() => this.handleEditButtonClick(true)}
            className="rush-btn rush-btn-edit"
          >
            Редактировать
          </Button>
          <Button
            onClick={this.handleDeleteButtonClick}
            className="rush-btn rush-btn-delete"
          >
            Удалить
          </Button>
          <input
        ...
        <SupplierTable data={tableData} handleRowClick={this.handleRowClick} />
        <SupplierModal
